...
class RNN(TensorFlowTrainable):
    def __init__(self, cell, num_units, embedding_dim, projecter, keep_prob, class_weights, **kwargs):
    ...
    def get_predictions(self):
        biases = self.get_biases(dim_out=4, name="biases")
        weights = self.get_weights(dim_in=self._num_units, dim_out=4, name="weights")
    ...
    def loss(self, targets):
        if self.hiddens is None:
            ...
            with tf.name_scope("loss") as scope:
                ...
                weighted_logits=tf.multiply(logits, tf.constant(self.class_weights))
                loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=weighted_logits, labels=targets))
